...
describe('CustomerList Component', () => {
  ...
  it('renders loading state initially', () => {
    customerAPI.getAll.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
  ...
  it('opens delete modal when delete button is clicked', async () => {
    ...
    expect(screen.getByText(/削除すると、この顧客に関連するすべての機器データも削除されます/)).toBeInTheDocument();
  });
  ...
  it('displays empty search results message when no customers match search', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByPlaceholderText('顧客名で検索...')).toBeInTheDocument();
    });
    ...
    expect(screen.getByText('検索条件に一致する顧客はありません。')).toBeInTheDocument();
  });
});