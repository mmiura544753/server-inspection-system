...
const importDevicesFromCsv = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          const deviceName = getColumnValue([
            "機器名",
            "device_name",
            "機器名",
            "@í¼",
          ]);
          const customerName = getColumnValue([
            "顧客名",
            "customer_name",
            "顧客名",
            "Úq¼",
          ...
          let rackNumber = null;
          const rackNumberInput = getColumnValue([
            "設置ラックNo",
            "rack_number",
            "ラックNo",
            "設置ラックNo.",
          ...
          if (rackNumberInput && rackNumberInput.toString().trim() !== "") {
            ...
            if (!isNaN(parsedValue)) {
              ...
              console.log(
                `ラックナンバーの変換に失敗: "${rackNumberInput}" は整数に変換できません`
              );
            }
          }

          const unitStartPos = getColumnValue([
            "ユニット開始位置",
            "unit_start_position",
          ]);
          const unitEndPos = getColumnValue([
            "ユニット終了位置",
            "unit_end_position",
          ]);

          const unit_start_position = unitStartPos
            ? parseInt(unitStartPos, 10)
            : null;
          const unit_end_position = unitEndPos
            ? parseInt(unitEndPos, 10)
            : null;

          const deviceType = getColumnValue(["機器種別", "device_type"]);
          const hardwareType = getColumnValue([
            "ハードウェアタイプ",
            "hardware_type",
          ]);
          ...
          const normalizedDeviceType = validDeviceTypes.includes(deviceType)
            ? deviceType
            : "サーバ";

          // ハードウェアタイプの検証
          const validHardwareTypes = ["物理", "VM"];
          const normalizedHardwareType = validHardwareTypes.includes(
            hardwareType
          )
            ? hardwareType
            : "物理";
          ...
          if (deviceId && !isNaN(parseInt(deviceId))) {
            ...
            if (existingDevice) {
              ...
              existingDevice.unit_start_position = unit_start_position;
              existingDevice.unit_end_position = unit_end_position;
              ...
              console.log(
                `デバイス更新完了: ID=${existingDevice.id}, 名前=${deviceName}`
              );
            ...
            const existingDevice = await Device.findOne({
              where: {
                ...
                unit_start_position: unit_start_position || "",
              },
            });
            ...
            if (existingDevice) {
              // 既存デバイスがある場合はスキップ
              console.log(
                `重複機器のためスキップ: ${deviceName} (${customer.customer_name})`
              );
              results.errors.push({
                row: row,
                error:
              ...
              try {
                ...
                const device = await Device.create(
                  {
                    ...
                    unit_start_position: unit_start_position || "",
                    unit_end_position: unit_end_position || "",
                ...
                console.log(
                  `デバイス作成完了: ID=${device.id}, 名前=${deviceName}`
                );
      ...
      console.log(
        `インポート完了: ${results.importedRows}/${results.totalRows}行をインポート`
      );
    ...
    res.status(500);
    throw new Error(
      `CSVのインポート中にエラーが発生しました: ${error.message}`
    );
  }
});
