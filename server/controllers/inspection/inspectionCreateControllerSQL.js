...
const createInspection = asyncHandler(async (req, res) => {
  ...
  for (const result of results) {
    ...
    const inspectionItem = await db.queryOne(itemSql, [
  ...
  return await db.transaction(async (conn) => {
    try {
      // 点検レコードの挿入 - device_idをserver_idに変更
      const insertInspectionSql = `
        INSERT INTO inspections 
        (inspection_date, start_time, end_time, inspector_name, server_id, status, created_at, updated_at) 
        VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
      `;
      ...
      const inspectionSql = `
        SELECT 
          ...
          i.server_id as device_id, i.status, i.created_at, i.updated_at,
