...
const updateInspection = asyncHandler(async (req, res) => {
  ...
  if (device_id) {
    ...
    fieldsToUpdate.server_id = device_id;
  }
  ...
  if (results && results.length > 0) {
    for (const result of results) {
      ...
      const inspectionItemExists = await db.queryOne(itemSql, [
    ...
    try {
      ...
      const updatedInspectionSql = `
        SELECT 
          ...
          i.server_id, i.status, i.created_at, i.updated_at,
          d.device_name, d.customer_id,
          c.customer_name
        FROM 
          inspections i
        JOIN 
          devices d ON i.server_id = d.id
        JOIN 
      ...
      const inspectionData = await conn.query(updatedInspectionSql, [
        inspectionId,
      ]);
      ...
      const formattedResults = resultsList.map((result) => {
        return {
          ...
          device_id: updatedInspection.server_id,
