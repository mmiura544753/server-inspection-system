 ...
 */
const { importInspectionItemsFromCsv } = require('../../../controllers/inspectionItem/inspectionItemImportController');
...
describe('inspectionItemImportController', () => {
  ...
  describe('importInspectionItemsFromCsv', () => {
    it('CSVファイルから点検項目を正常にインポートできる', async () => {
      ...
      InspectionItem.findOne.mockResolvedValueOnce(null).mockResolvedValueOnce(null); // 重複チェック（2回）
      InspectionItem.create.mockResolvedValueOnce(mockInspectionItem1).mockResolvedValueOnce(mockInspectionItem2); // 点検項目作成（2回）
      ...
      expect(csvParse.parse).toHaveBeenCalledWith(mockCSVContent, expect.any(Object));
      ...
      expect(res.json).toHaveBeenCalledWith(expect.objectContaining({
        message: expect.stringContaining('2/2 件のデータをインポートしました'),
    ...
    it('既存の点検項目をIDに基づいて更新できる', async () => {
      ...
      const mockCSVContent = 'ID,点検項目,サーバ名,ラックNo.,ユニット,機種,顧客名\n1,CPU状態確認,サーバー1,1,42～44,Model A,顧客A';
        ...
        data: expect.objectContaining({
          ...
          errors: expect.arrayContaining([
            expect.objectContaining({
              error: expect.stringContaining('指定されたID: 999の点検項目が存在しません')
            })
          ])
        })
      }));
    ...
    it('CSVパース中にエラーが発生した場合はエラーを返す', async () => {
      ...
      // エラーをキャッチするためのnext関数をセットアップ
      const next = jest.fn(error => {
        expect(error.message).toContain('CSVのインポート中にエラーが発生しました');
      });
